    ...
    /** @scope M.SplitView.prototype */ {
        ...
        themeUpdate: function() {
            ...
            if (M.Environment.getWidth() > M.Environment.getHeight()) {
                ...
                $('.tmp-splitview-content-toolbar').css('float', 'right').css('width', Math.floor(width * 0.7) - (parseInt($('#' + this.content.id).css('padding-right')) + parseInt($('#' + this.content.id).css('padding-left'))) + 'px');
                /* portrait mode */
            } else {
...
//            }
        ...
        initializeVar: function() {
            var page = this.getPage();
            this.headerheight = $('#' + page.id + ' .ui-header').height();
            this.footerheight = $('#' + page.id + ' .ui-footer').height();
        ...
        orientationDidChange: function() {
            var start = M.Date.create().date.getTime()
        ...
        /**
         * caching function for getParentPage
